...
var View = {
    ...
    applyZoom: function() {
        var width = this.numCols * this.nodeSize,
            height = this.numRows * this.nodeSize,
        ...
        if (this.$zoomLevel) {
            this.$zoomLevel.text(Math.round(zoomLevel * 100) + '%');
        }
    },
    setZoom: function(zoomLevel) {
        zoomLevel = Math.max(this.minZoomLevel, Math.min(this.maxZoomLevel, zoomLevel));
    ...
    toGridCoordinate: function(pageX, pageY) {
        ...
        return [
            ...
            Math.floor((pageY - offset.top) / zoomLevel / this.nodeSize)
        ];
    },
    /**
    ...
    showStats: function(opts) {
        var texts = [
            'độ dài: ' + Math.round(opts.pathLength * 100) / 100,
            'thời gian: ' + opts.timeSpent + 'ms',
            'số thao tác: ' + opts.operationCount
        ];
